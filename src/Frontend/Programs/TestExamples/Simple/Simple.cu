...
{
    const int frameID = param.frameID;
    ...
    const int r = (int(fabsf(sinf(0.02f * (ix + 0.2f * frameID))) * 230) % 256);
    const int b = (int(fabsf(sinf(0.02f * (iy + 0.2f * frameID))) * 230) % 256);

    const int g =
        (int(fabsf(sinf(0.02f * (ix + iy + 0.5f * frameID))) * 230) % 256);
